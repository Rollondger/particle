...
// #include <gl\glaux.h>		// Header File For The Glaux Library, outdated, unused atm.

#define PARTICLECOUNT	9000		// number of particles, not counting stars or planets, 9000 works well.
#define lifeset			1000		// lifetime of particles, unused atm.
#define slowdown		4000		// speed controler, 4000 is a good base
...
#define spawndist		0			// dist each box is from one another, defualt is 60
#define rotation        1000		// speed at which particles spawn at, with each of the four colors moving in a diffrent direction.
...
//ULONGLONG timer;			// used as a timer with GetTickCount64();  this is millisecond prescise.
...
*/
	...
	for(i = 0; i < PARTICLECOUNT; i ++)			//startup section, make them active, spawn them, and fire them into the arena
	{
		...
		particle[i].xv = 0.0; // this are the speed vectors for the particles, by making this group move down, and the other groups move in other directions
...
int drawparticle_type1(GLvoid)
{
			...
			glColor3f(particle[i].r, particle[i].g, particle[i].b);				// DRAW THEM PARTICLES!
		//	glBegin(GL_POLYGON);
			glBegin(GL_POINTS);			// try switching back and forth once debugger/prformance log is working
		//	glVertex3f(particle[i].x + 0.2f, particle[i].y, particle[i].z);		// they are little triangles atm, but someday they will be textured points
		//	glVertex3f(particle[i].x, particle[i].y + 0.2f, particle[i].z);
		//	glVertex3f(particle[i].x + 1.0f, particle[i].y + 1.0f, particle[i].z);	
...
//	ofstream myfile ("example.txt");
	...
	for(loop = 0; loop < PARTICLECOUNT; loop++)
		...
		dist = (sqrt(((particle[i].x + paticle[(i+loop)].x ) * particle[i].x) + (particle[i].y * particle[i].y)));



		
		{
			particle[i].xv += ((particle[i].x / dist ) * ((1/dist * dist) * (gravity * (particle[i].mass))));   
			particle[i].yv += ((particle[i].y / dist ) * ((1/dist * dist) * (gravity * (particle[i].mass))));
...
*/
	...
	if(particle[i].x < 2)			// colision at center point, this is needed to destory particles on colision, will
	{								// later be dynamicly upgraded to destroy particles on contact with each other
		if(particle[i].x > -2)		// and at a radius to be decided dynamicly as well (particle[i].radius) or something mass based.
		{
			if(particle[i].y < 2)
			{
				if(particle[i].y > -2)
...
//	}
	...
	if (!CreateGLWindow("My Awesome Particle System, Thanks Nehe!",1440,900,32,fullscreen))
	{
